...
cityInput.addEventListener('input', (e) => {
    ...
    searchTimeout = setTimeout(() => {
        suggestionsContainer.innerHTML = '';
        const filteredCities = cityList.filter(city => city.lower.startsWith(query.toLowerCase())).map(city => city.name);
...
async function fetchWeatherData(city) {
    ...
    try {
        ...
        loadingScreen.classList.add('hidden');
        showNotification('An unexpected error occurred. Please check your internet connection and try again.');
...
function displayForecast(forecastData, container) {
    ...
    if (!container) {
        ...
        forecastDiv.innerHTML = '<h3>5-Day Forecast</h3><div class="forecast-items"></div>';
    ...
    forecastData.list.forEach(item => {
        ...
        if (!dailyForecasts[day] || Math.abs(hours - 12) < Math.abs((new Date(dailyForecasts[day].dt * 1000).getHours()) - 12)) {
...
function setBackgroundByWeather(weatherMain, iconCode) {
    body.classList.remove('weather-clear','weather-clouds','weather-rain','weather-snow','weather-thunderstorm','weather-mist','weather-night');
...
function displayAQIData(aqiData) {
    ...
    if (aqiElement && aqiData.list && aqiData.list.length > 0) {
        ...
        aqiElement.innerHTML = `<h3>Air Quality</h3><div class="aqi-indicator ${aqiClass}"><div class="aqi-value">${aqiValue}</div><div class="aqi-text">${aqiText}</div></div><div class="aqi-description"><p>${getAQIDescription(aqiValue)}</p></div>`;
...
document.addEventListener('DOMContentLoaded', () => {
    fetch('countries.json').then(response => response.json()).then(data => {
        ...
        cityList = allCities.map(city => ({name: city, lower: city.toLowerCase()}));
    }).catch(error => {
        console.error(error);
    });
